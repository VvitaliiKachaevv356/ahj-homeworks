  ...
  init() {
    ...
    const handlerClick = (e) => {
      ...
      fetch("https://eventsource-websockets-backendba.onrender.com/new-user", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // добавляем заголовок для JSON
        },
        body: JSON.stringify({ name: this.you }), // исправлено
      })
        ...
        .then((data) => {
          if (data.status === "error") {
            ...
            this.formNickname.removeEventListener("submit", handlerClick); // удаляем обработчик формы
            ...
            this.closingPage(data.user.name); // закрытие страницы или другие действия
          }
        })
        .catch((error) => {
